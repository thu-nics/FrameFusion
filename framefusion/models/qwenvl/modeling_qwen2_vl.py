...
from transformers.models.qwen2_vl.modeling_qwen2_vl import QWEN2_VL_INPUTS_DOCSTRING, Qwen2VLCausalLMOutputWithPast, _CONFIG_FOR_DOC
import torch
...
def forward(
    ...
    if inputs_embeds.shape[1] != 1:
        patch_num = (video_grid_thw[0,1] * video_grid_thw[0,2])/(self.visual.config.spatial_merge_size * self.visual.config.spatial_merge_size)
        patch_num = int(patch_num)
        image_token_length = video_embeds.shape[0]
        original_length = inputs_embeds.shape[1]
        image_token_start_index = torch.where(input_ids[0] == self.config.video_token_id)[0][0]
        image_token_end_index = torch.where(input_ids[0] == self.config.video_token_id)[0][-1]
