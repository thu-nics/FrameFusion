...
from transformers.models.qwen2.modeling_qwen2 import Qwen2Attention, Qwen2SdpaAttention, Qwen2DecoderLayer, Qwen2Model
...
def replace_Qwen2_forward(model, mode="merge_then_fastv_cost_given", **kwargs):
    ...
    elif mode=="merge_then_fastv_cost_given":
        merge_then_fastv_cost_given_kwargs = {
            "cost": kwargs.get("cost", 0.3),
            "similarity_lower_bound": kwargs.get("similarity_lower_bound", 0.5),
            "ratio_lower_bound": kwargs.get("ratio_lower_bound", 0.1)
        }
        print(f"Config\n{merge_then_fastv_cost_given_kwargs}")

        replace_Qwen2_merge_then_fastv_cost_given(
            model,
            **merge_then_fastv_cost_given_kwargs
        )
...
def replace_minicpmv_forward(model, mode="merge_then_fastv_cost_given", **kwargs):
    print(f"replace_minicpmv_forward mode: {mode} and kwargs: {kwargs}")
    if mode=="merge_then_fastv_cost_given":
        merge_then_fastv_kwargs = {
            "cost": kwargs.get("cost", 0.3),
            "similarity_lower_bound": kwargs.get("similarity_lower_bound", 0.5),
            "ratio_lower_bound": kwargs.get("ratio_lower_bound", 0.1)
        ...
        replace_minicpmv_merge_then_fastv_cost_given(
            model,
            **merge_then_fastv_kwargs
        )
    elif mode=="fastv":
...
def replace_Qwen2_decode_layer_merging_forward(decoder_layer):
    decoder_layer.forward=MethodType(Qwen2DecoderLayer_merging_forward, decoder_layer)

def replace_Qwen2SdpaAttenion_merging_forward(qwen2_attention_instance, model):
...
def replace_Qwen2Model_merging_forward(model):
    model.forward = MethodType(Qwen2Model_merging_forward, model)
...
def replace_Qwen2_merge_then_fastv_cost_given(model, cost = 0.3, similarity_lower_bound = 0.5, ratio_lower_bound = 0.1):
    # model.framefusion.cost = cost
    # model.framefusion.similarity_lower_bound = similarity_lower_bound
    # model.frsmefusion.ratio_lower_bound = ratio_lower_bound 
    model.framefusion = FrameFusion(cost, similarity_lower_bound, ratio_lower_bound)
    
    if isinstance(model.model, Qwen2Model):
        model.model.forward = MethodType(partial(Qwen2Model_merge_then_fastv_cost_given_forward, model=model), model.model)
    for i, decoder_layer in enumerate(model.model.layers):
        if isinstance(decoder_layer, Qwen2DecoderLayer):
...
def replace_minicpmv_merge_then_fastv_cost_given(model, cost = 0.3, similarity_lower_bound = 0.5, ratio_lower_bound = 0.1):
    
    model.framefusion = FrameFusion(cost, similarity_lower_bound, ratio_lower_bound)
    
    if isinstance(model.llm.model, Qwen2Model):
        model.llm.model.forward = MethodType(partial(Qwen2Model_merge_then_fastv_cost_given_forward, model=model), model.llm.model)
...
def compute_needed_fastv_r(sparsity_list, cost, num_layers = 28):
...
def compute_sparsity_upper_bound(sparsity_list, cost, num_layers = 28):
    list_length = len(sparsity_list)
    s = 1
    total_calcution =0
    for i in range(list_length):
