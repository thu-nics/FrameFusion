...
from framefusion.models.qwen2.modeling_qwen2 import replace_minicpmv_forward, replace_Qwen2_forward
...
def get_token_type(model):
    ...
    # LlavaVideo Model
        ...
        model.prepare_inputs_labels_for_multimodal = MethodType(prepare_inputs_labels_for_multimodal_get_token_type, model)
...
def apply_framefusion(model, cost, similarity_lower_bound, ratio_lower_bound):
    ...
    if isinstance(model,LlavaNextVideoForConditionalGeneration):
        model._merge_input_ids_with_image_features = MethodType(_merge_input_ids_with_image_features_get_token_type, model)
        replace_Qwen2_forward(model, mode="merge_then_fastv_cost_given", cost=cost, similarity_lower_bound=similarity_lower_bound, ratio_lower_bound=ratio_lower_bound)
    ...
    # MiniCPM Model
        ...
        model.get_vllm_embedding = MethodType(get_vllm_embedding, model)
        replace_minicpmv_forward(model, mode="merge_then_fastv_cost_given", cost=cost, similarity_lower_bound=similarity_lower_bound, ratio_lower_bound=ratio_lower_bound)
    else:
        raise NotImplementedError
